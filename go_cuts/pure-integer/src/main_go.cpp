/**@file   main_vrp.cpp
 * @brief  Main program file for GO cut separation
...
)
{
    ...
    if(!outFile.empty()){
        ...
        if(withCPC)
        {
            sol_file << "Seed;SolTime;nNodes;Gap;LPIter;nSepCalls;sepTime;nCuts;nCutsRoot;nFails" << std::endl;
        }else {
        ...
        if(withCPC)
        {
            sol_file << ";" << SCIPconshdlrGetNSepaCalls(CPC) << ";" << SCIPconshdlrGetSepaTime(CPC) << ";" << obj->nCuts_;
            sol_file << ";" << obj->nCutsRoot_ << ";" << obj->nFails_;
        }
        sol_file << std::endl;
    }
